	...
	for dir in io.popen([[dir "./" /b]]):lines() do
		for index,value in ipairs(dir) do
			if ( string.find(value, name,1,true) ) then
				...
				print("	cant find file in mod fodler by partial match: "..name)
			end
		end
	end
end
elseif (pathSeperator == '/') then
	...
	print("IM SORRY BUT UNTILL MINETEST SUPORTS REQUIRE OR HAS A WAY TO HANDLE LOADING OR INDEXING FILES BETTER THIS MOD WILL NOT WORK ON YOUR OS")
end
